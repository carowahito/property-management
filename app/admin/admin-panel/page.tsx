...
export default function AdminPanelPage() {
  ...
  const getColorClasses = (color: string) => {
    switch (color) {
      ...
      default: return 'bg-neutral-100 text-neutral-800 hover:bg-neutral-200'
    }
  }
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="bg-gradient-to-r from-danger-600 to-danger-700 rounded-lg p-8 text-white">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-danger-100">System administration and configuration</p>
          </div>
          <div className="text-right">
            <p className="text-sm text-danger-100">Logged in as</p>
            <p className="font-semibold">{currentUser.name}</p>
            <span className="inline-block mt-2 px-3 py-1 bg-danger-500 rounded-full text-xs font-medium">
              Administrator
            </span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white rounded-xl shadow-md p-6 border border-neutral-200">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-neutral-500 mb-1">Team Members</p>
              <p className="text-3xl font-bold text-neutral-900">{systemStats.totalUsers}</p>
              <p className="text-xs text-success-600 mt-1">{systemStats.activeUsers} active</p>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-md p-6 border border-neutral-200">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-neutral-500 mb-1">API Calls</p>
              <p className="text-3xl font-bold text-neutral-900">{systemStats.apiCalls.toLocaleString()}</p>
        ...
        <div className="bg-white rounded-xl shadow-md p-6 border border-neutral-200">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-neutral-500 mb-1">System Uptime</p>
              <p className="text-3xl font-bold text-neutral-900">{systemStats.uptime}</p>
              <p className="text-xs text-success-600 mt-1">Operational</p>
            </div>
      ...
      {/* Admin Modules Grid */}
      <div>
        <h2 className="text-xl font-bold text-neutral-900 mb-4">Administration Modules</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {adminModules.map((module) => (
            <Link
              key={module.id}
              href={module.href}
              className="bg-white rounded-lg border-2 border-neutral-200 p-6 hover:border-neutral-300 hover:shadow-lg transition group"
            >
              <div className="flex items-start justify-between mb-4">
                <div className={`text-4xl p-3 rounded-xl ${getColorClasses(module.color)}`}>
                  {module.icon}
                </div>
                <svg className="w-5 h-5 text-neutral-400 group-hover:text-neutral-600 transition" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="bg-white rounded-lg border border-neutral-200 p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold text-neutral-900">Recent System Activity</h2>
          <Link href="/admin/admin-panel/audit" className="text-sm text-primary-600 hover:text-primary-800">
            View All →
          </Link>
        </div>
        <div className="space-y-3">
          {recentActivity.map((activity) => (
            <div key={activity.id} className="flex items-center justify-between p-3 bg-neutral-50 rounded-lg">
              ...
              <span className="text-xs text-neutral-500">{formatDateTime(activity.date)}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <button className="p-4 bg-white border-2 border-dashed border-neutral-300 rounded-lg hover:border-primary-500 hover:bg-primary-50 transition text-center group">
          <span className="text-3xl mb-2 block">➕</span>
          <p className="font-medium text-neutral-900 group-hover:text-primary-600">Add Team Member</p>
        </button>
        ...
        <button className="p-4 bg-white border-2 border-dashed border-neutral-300 rounded-lg hover:border-success-500 hover:bg-success-50 transition text-center group">
          <span className="text-3xl mb-2 block">💾</span>
          <p className="font-medium text-neutral-900 group-hover:text-success-600">Backup System</p>
        </button>
      </div>
    </div>
  )
