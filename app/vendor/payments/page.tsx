    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-neutral-600 mt-1">Track your earnings and payment status</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-surface shadow rounded-lg p-4">
          ...
          <p className="text-2xl font-bold text-success-600">KES {(totalPaid / 1000).toFixed(0)}K</p>
        </div>
        <div className="bg-surface shadow rounded-lg p-4">
          <p className="text-sm text-neutral-600">Pending</p>
          <p className="text-2xl font-bold text-warning-600">KES {(totalPending / 1000).toFixed(0)}K</p>
        </div>
        <div className="bg-surface shadow rounded-lg p-4">
        ...
        <div className="divide-y divide-neutral-200">
          {filteredPayments.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-neutral-500">No {activeTab} payments</p>
            </div>
          ) : (
            filteredPayments.map((payment) => (
              <div key={payment.id} className="p-6 hover:bg-neutral-50">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="font-semibold text-neutral-900">{payment.id}</h3>
                    ...
                    <p className="text-sm text-neutral-600 mb-3">Property: {payment.property}</p>
                      ...
                      <div className="flex items-center gap-4 text-sm mb-2">
                        ...
                        {payment.reference && (
                          <>
                            ...
                            <span className="font-mono text-xs bg-neutral-100 px-2 py-1 rounded">{payment.reference}</span>
                          </>
                        )}
                      </div>
                    )}
                    ...
                    <div className="flex items-center gap-4 text-sm text-neutral-600">
                      ...
                      {payment.paymentDate && (
                        <span className="text-success-600">Paid: {new Date(payment.paymentDate).toLocaleDateString()}</span>
                      )}
                    </div>
                  </div>
                  
                  <div className="ml-4 text-right">
                    <p className="text-2xl font-bold text-neutral-900">KES {payment.amount.toLocaleString()}</p>
                    ...
                    {payment.status === 'Paid' && (
                      <p className="text-sm text-success-600 mt-1">✓ Payment Received</p>
                    )}
                    {payment.status === 'Pending' && (
                      <p className="text-sm text-warning-600 mt-1">Awaiting Payment</p>
                    )}
                  </div>
                </div>
              </div>
      ...
      <div className="bg-surface shadow rounded-lg p-6">
        <h2 className="text-xl font-semibold text-neutral-900 mb-4">Payment Summary</h2>
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            <span className="text-neutral-600">Total Invoiced:</span>
            <span className="font-semibold text-neutral-900">KES {totalEarnings.toLocaleString()}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-neutral-600">Total Paid:</span>
            <span className="font-semibold text-success-600">KES {totalPaid.toLocaleString()}</span>
          </div>
          <div className="flex justify-between items-center pt-3 border-t">
            <span className="text-neutral-900 font-medium">Outstanding Balance:</span>
            <span className="font-bold text-warning-600 text-lg">KES {totalPending.toLocaleString()}</span>
          </div>
        </div>
      </div>
    </div>
