  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Receipts & Invoices</h1>
          <p className="text-gray-600 mt-1">Upload and track your expense receipts</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white shadow rounded-lg p-4">
          ...
          <p className="text-2xl font-bold text-gray-900">KES {(totalAmount / 1000).toFixed(0)}K</p>
        </div>
        <div className="bg-white shadow rounded-lg p-4">
          <p className="text-sm text-gray-600">Approved</p>
          <p className="text-2xl font-bold text-green-600">KES {(approvedAmount / 1000).toFixed(0)}K</p>
        </div>
        <div className="bg-white shadow rounded-lg p-4">
            ...
            <div key={receipt.id} className="p-6 hover:bg-gray-50">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-600 mb-1">Job: {receipt.jobId} - {receipt.jobDescription}</p>
                  <p className="text-sm text-gray-600 mb-2">Property: {receipt.property}</p>
                  
                  {receipt.notes && (
                    <p className="text-sm text-gray-700 mb-3">{receipt.notes}</p>
                  )}
                  ...
                  <div className="flex flex-wrap gap-2 mb-2">
                    {receipt.files.map((file, idx) => (
                      <div key={idx} className="flex items-center gap-2 bg-gray-100 px-3 py-1 rounded">
                        ...
                        <a href={file.url} className="text-orange-600 hover:text-orange-800 text-xs">View</a>
                      </div>
                    ))}
                  </div>
                  
                  <p className="text-sm text-gray-500">Submitted: {new Date(receipt.date).toLocaleDateString()}</p>
                </div>
                
                <div className="text-right ml-4">
                  <p className="text-2xl font-bold text-gray-900">KES {receipt.amount.toLocaleString()}</p>
                </div>
              </div>
            </div>
          ))}
