  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Work Evidence & Documentation</h1>
          <p className="text-gray-600 mt-1">Review before/after photos and videos of repairs on your properties</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white shadow rounded-lg p-4">
          ...
          <p className="text-2xl font-bold text-orange-600">{inProgressJobs}</p>
        </div>
        <div className="bg-white shadow rounded-lg p-4">
          <p className="text-sm text-gray-600">Completed & Verified</p>
      ...
      <div className="space-y-6">
        {filteredEvidences.map(([jobId, evidences]) => {
          ...
          return (
            <div key={jobId} className="bg-white shadow rounded-lg p-6">
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <h3 className="text-xl font-semibold text-gray-900">{job.workDescription}</h3>
                  ...
                  <div className="flex items-center gap-4 text-sm text-gray-600 mb-3">
                    ...
                    {job.completedDate && (
                      <span className="text-green-600">Completed: {new Date(job.completedDate).toLocaleDateString()}</span>
                    )}
                  </div>
                </div>
              </div>
              ...
              {beforePhotos.length > 0 && (
                <div className="mb-6">
                  <h4 className="font-semibold text-gray-900 mb-3 flex items-center gap-2">
                    <span className="text-xs px-2 py-1 rounded bg-red-100 text-red-800">Before Work</span>
                    <span className="text-sm text-gray-600">({beforePhotos[0].files.length} files)</span>
                  </h4>
                  ...
                  <p className="text-sm text-gray-700 mt-2">{beforePhotos[0].description}</p>
                </div>
              )}
              ...
              {duringPhotos.length > 0 && (
                <div className="mb-6">
                  <h4 className="font-semibold text-gray-900 mb-3 flex items-center gap-2">
                    <span className="text-xs px-2 py-1 rounded bg-blue-100 text-blue-800">Work in Progress</span>
                    <span className="text-sm text-gray-600">({duringPhotos[0].files.length} files)</span>
                  </h4>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <p className="text-sm text-gray-700 mt-2">{duringPhotos[0].description}</p>
                </div>
              )}
              ...
              {afterPhotos.length > 0 && (
                <div className="mb-4">
                  <h4 className="font-semibold text-gray-900 mb-3 flex items-center gap-2">
                    <span className="text-xs px-2 py-1 rounded bg-green-100 text-green-800">Completed Work</span>
                    <span className="text-sm text-gray-600">({afterPhotos[0].files.length} files)</span>
                  </h4>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <p className="text-sm text-gray-700 mt-2">{afterPhotos[0].description}</p>
                </div>
              )}
