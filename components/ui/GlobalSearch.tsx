...
const TYPE_CONFIG = {
  landlord:  { label: 'Landlords',  icon: '🏢', color: 'bg-green-100 text-green-700' },
  property:  { label: 'Properties', icon: '🏠', color: 'bg-blue-100 text-blue-700' },
  unit:      { label: 'Units',      icon: '🔑', color: 'bg-purple-100 text-purple-700' },
  tenant:    { label: 'Tenants',    icon: '👤', color: 'bg-orange-100 text-orange-700' },
}

function statusColor(status: string) {
  if (status === 'ACTIVE' || status === 'OCCUPIED') return 'text-green-600'
  if (status === 'INACTIVE' || status === 'VACANT') return 'text-gray-500'
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      <button
        ...
        className="flex items-center gap-2 px-3 py-2 bg-gray-100 hover:bg-gray-200 rounded-lg transition text-sm text-gray-500 w-64"
      >
      ...
      {/* Dropdown panel */}
      {open && (
        <div className="absolute left-0 top-full mt-2 w-[480px] bg-white rounded-xl shadow-2xl border border-gray-200 z-50 overflow-hidden">
          {/* Input */}
          ...
          {/* Footer */}
          <div className="px-3 py-2 border-t border-gray-100 flex items-center gap-3 text-xs text-gray-400">
